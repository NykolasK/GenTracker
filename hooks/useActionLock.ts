...
export function useActionLock(options: ActionLockOptions = {}) {
  ...
  const executeAction = useCallback(
    async (action: () => Promise<any>, actionName?: string): Promise<any | null> => {
      // Verifica se está em cooldown
      if (state.isLocked) {
        if (showFeedback) {
          console.warn(`Ação "${actionName || "desconhecida"}" bloqueada - em cooldown`)
        }
        return null
      }
      ...
      if (activeActions.current >= maxConcurrent) {
        if (showFeedback) {
          console.warn(`Ação "${actionName || "desconhecida"}" bloqueada - muitas ações simultâneas`)
        }
        return null
      }
